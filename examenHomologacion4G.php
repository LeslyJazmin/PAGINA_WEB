...
function inicializarSesion() {
    if (!isset($_SESSION['intentos_por_curso_Homologacion4G'])) {
        $_SESSION['intentos_por_curso_Homologacion4G'] = [];
    }

    // ID 29 corresponde al curso de Homologación 4G en proceso smaw
    if (!isset($_SESSION['intentos_por_curso_Homologacion4G'][29])) {
        $_SESSION['intentos_por_curso_Homologacion4G'][29] = 0;
    }

    if (!isset($_SESSION['notas_por_curso_Homologacion4G'])) {
        $_SESSION['notas_por_curso_Homologacion4G'] = [];
    }
    if (!isset($_SESSION['notas_por_curso_Homologacion4G'][29])) {
        $_SESSION['notas_por_curso_Homologacion4G'][29] = [];
    }
    if (!isset($_SESSION['examen_completado_por_curso_Homologacion4G'][29])) {
        $_SESSION['examen_completado_por_curso_Homologacion4G'][29] = false;
    }
    ...
    $_SESSION['examen_completado_por_curso_Homologacion4G'][29] = false;
    $_SESSION['nota_final_por_curso_Homologacion4G'][29] = 0;
    $_SESSION['estado_final_por_curso_Homologacion4G'][29] = '';
    $_SESSION['nota_mas_alta_por_curso_Homologacion4G'][29] = 0;
    $_SESSION['id_curso'] = 29; // Curso de Homologación 4G en proceso smaw
}

// Inicializar sesión si no existe o si se solicita reiniciar
if (!isset($_SESSION['intentos_por_curso_Homologacion4G']) || isset($_POST['reiniciar'])) {
    inicializarSesion();
}
...
if (isset($_POST['iniciar_examen'])) {
    if ($_SESSION['intentos_por_curso_Homologacion4G'][29] < 3) {
        $_SESSION['examen_iniciado'] = true;
        $_SESSION['tiempo_inicio'] = time();
        $_SESSION['tiempo_restante'] = $duracion_examen;
        header("Location: examenHomologacion4G.php");
        exit();
    }
}
...
// Obtener las preguntas del examen para el curso de Homologación 4G (id_curso = 29)
$preguntas = obtenerPreguntas(29);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['respuestas'])) {
    if (tiempoExpirado()) {
        $_SESSION['notas_por_curso_Homologacion4G'][29][] = 0;
        ...
        $_SESSION['notas_por_curso_Homologacion4G'][29][] = $nota;
        $_SESSION['examen_iniciado'] = false;
        ...
        if ($nota > $_SESSION['nota_mas_alta_por_curso_Homologacion4G'][29]) {
            $_SESSION['nota_mas_alta_por_curso_Homologacion4G'][29] = $nota;
        }

        // Incrementar intento actual en sesión
        $_SESSION['intentos_por_curso_Homologacion4G'][29]++;
        $intento_actual = $_SESSION['intentos_por_curso_Homologacion4G'][29];

        // Actualizar base de datos
        if (isset($_SESSION['nota_mas_alta_por_curso_Homologacion4G'][29]) && isset($_SESSION['DNI'])) {
            $conn = new mysqli('localhost', 'root', '', 'usuario');
            if ($conn->connect_error) {
                die("Error de conexión: " . $conn->connect_error);
            }
            ...
            $DNI = $_SESSION['DNI'];
            $nota_mas_alta = $_SESSION['nota_mas_alta_por_curso_Homologacion4G'][29];

            // Verificar inscripción
            $sqlVerificar = "SELECT id_curso FROM inscripciones WHERE DNI = ? AND id_curso = 29";
        ...
        if ($intento_actual < 3) {
            $mensaje = "Has completado el intento $intento_actual con $nota puntos. Estado: " . $_SESSION['estado_final_por_curso_Homologacion4G'][29];
        } else {
            $mensaje = "En este último intento tu nota es $nota. La nota final de tus 3 intentos ha sido " . $_SESSION['nota_mas_alta_por_curso_Homologacion4G'][29];
            $_SESSION['examen_completado_por_curso_Homologacion4G'][29] = true;
        }
    }
}
...
if (isset($_POST['reiniciar'])) {
    ...
    $sql = "UPDATE inscripciones SET `intentose1` = 0, `intentose2` = 0, `intentose3` = 0, `examen_final` = 0 WHERE DNI = ? AND id_curso = 29";
    $stmt = $conn->prepare($sql);
...
<head>
    ...
    <title>Examen de Homologación 4G en proceso smaw</title>
...
<body>
    ...
    <div class="container">
        <h1>Examen de Homologación 4G en proceso smaw</h1>
    <?php if ($_SESSION['examen_completado_por_curso_Homologacion4G'][29]): ?>
    <?php if ($_SESSION['estado_final_por_curso_Homologacion4G'][29] == 'APROBADO'): ?>
            <div class="mensaje">
                Felicidades! Ya has aprobado el examen. No necesitas realizar más intentos.
            </div>
            <form method="post">
                <button type="button" value="Volver a mi curso" class="volver" onclick="window.location.href='Homologacion4G.php'">Volver a mi curso</button>
            </form>

    <?php else: ?>
            <?php if ($_SESSION['intentos_por_curso_Homologacion4G'][29] < 3): ?>
                <div class="mensaje">
                    Has reprobado el examen con <?php echo $_SESSION['nota_final_por_curso_Homologacion4G'][29]; ?> puntos. Te queda
                    <?php echo 3 - $_SESSION['intentos_por_curso_Homologacion4G'][29]; ?> intento(s). Puedes intentar nuevamente.
                </div>
                <div class="mensaje">
                    El puntaje más alto obtenido hasta ahora es <?php echo max($_SESSION['notas_por_curso_Homologacion4G'][29] ?? [0]); ?> puntos.
                </div>
                <button onclick="mostrarModal()" class="modal-button">Continuar con el siguiente intento</button>
            <?php else: ?>
                <form method="post">
                    <input type="button" value="Volver a mi curso" class="volver" onclick="window.location.href='Homologacion4G.php'">
                </form>
                <div class="mensaje">
...
<?php elseif ($_SESSION['intentos_por_curso_Homologacion4G'][29] < 3): ?>
...
<div id="inicioModal" class="modal">
    <div class="modal-content">
        ...
        <div class="button-container">
            ...
            <button type="button" class="modal-button" onclick="window.location.href='Homologacion4G.php'">Volver atrás</button>
        </div>
        </form>
    </div>
</div>
