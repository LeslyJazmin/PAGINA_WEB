...
function inicializarSesion() {
    if (!isset($_SESSION['intentos_curso_seguridadTC'])) {
        $_SESSION['intentos_curso_seguridadTC'] = [];
    }
    
    // ID 2 corresponde al curso de primeros auxilios
    if (!isset($_SESSION['intentos_curso_seguridadTC'][26])) {
        $_SESSION['intentos_curso_seguridadTC'][26] = 0;
    }
    
    if (!isset($_SESSION['notas_curso_seguridadTC'])) {
        $_SESSION['notas_curso_seguridadTC'] = [];
    }
    if (!isset($_SESSION['notas_curso_seguridadTC'][26])) {
        $_SESSION['notas_curso_seguridadTC'][26] = [];
    }
    if (!isset($_SESSION['videotest_completado_curso_seguridadTC'][26])) {
        $_SESSION['videotest_completado_curso_seguridadTC'][26] = false;
    }    
    ...
    $_SESSION['videotest_completado_curso_seguridadTC'][26] = false;
    $_SESSION['nota_final_curso_seguridadTC'][26] = 0;
    $_SESSION['estado_final_curso_seguridadTC'][26] = '';
    $_SESSION['nota_mas_alta_curso_seguridadTC'][26] = 0;
    $_SESSION['id_curso'] = 26; // Curso de Uso y manejo de Extintores
}

// Inicializar sesión si no existe o si se solicita reiniciar
if (!isset($_SESSION['intentos_curso_seguridadTC']) || isset($_POST['reiniciar'])) {
    inicializarSesion();
}
...
if (isset($_POST['iniciar_videotest'])) {
    if ($_SESSION['intentos_curso_seguridadTC'][26] < 3) {
        $_SESSION['videotest_iniciado'] = true;
        $_SESSION['tiempo_inicio'] = time();
        $_SESSION['tiempo_restante'] = $duracion_examen;
        header("Location: videotestseguridadTC.php");
        exit();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['respuestas'])) {
    if (tiempoExpirado()) {
        $_SESSION['notas_curso_seguridadTC'][26][] = 0;
        $_SESSION['estado_final_curso_seguridadTC'][26] = 'REPROBADO';
        ...
        $_SESSION['notas_curso_seguridadTC'][26][] = $nota;
        $_SESSION['videotest_iniciado'] = false;

        // Evaluar estado
        if ($nota >= 13) {
            $_SESSION['estado_final_curso_seguridadTC'][26] = 'APROBADO';
            $_SESSION['nota_final_curso_seguridadTC'][26] = max($_SESSION['notas_curso_seguridadTC'][26]);
            $_SESSION['videotest_completado_curso_seguridadTC'][26] = true;
        } else {
            $_SESSION['estado_final_curso_seguridadTC'][26] = 'REPROBADO';
        }

        // Actualizar la mejor nota
        if ($nota > $_SESSION['nota_mas_alta_curso_seguridadTC'][26]) {
            $_SESSION['nota_mas_alta_curso_seguridadTC'][26] = $nota;
        }

        // Incrementar intento actual en sesión
        $_SESSION['intentos_curso_seguridadTC'][26]++;
        $intento_actual = $_SESSION['intentos_curso_seguridadTC'][26];

        // Actualizar base de datos
        if (isset($_SESSION['nota_mas_alta_curso_seguridadTC'][26]) && isset($_SESSION['DNI'])) {
            $conn = new mysqli('localhost', 'root', '', 'usuario');
            if ($conn->connect_error) {
                die("Error de conexión: " . $conn->connect_error);
            }
            ...
            $DNI = $_SESSION['DNI'];
            $nota_mas_alta = $_SESSION['nota_mas_alta_curso_seguridadTC'][26];
        ...
        if ($intento_actual < 3) {
            $mensaje = "Has completado el intento $intento_actual con $nota puntos. Estado: " . $_SESSION['estado_final_curso_seguridadTC'][26];
        } else {
            $mensaje = "En este último intento tu nota es $nota. La nota final de tus 3 intentos ha sido " . $_SESSION['nota_mas_alta_curso_seguridadTC'][26];
            $_SESSION['videotest_completado_curso_seguridadTC'][26] = true;
        }
    }
}
...
if (isset($_POST['reiniciar'])) {
    ...
    $sql = "UPDATE inscripciones SET `intento1` = 0, `intento2` = 0, `intento3` = 0, `nota_videotest` = 0 WHERE DNI = ? AND id_curso = 26";
    $stmt = $conn->prepare($sql);
...
<head>
    ...
    <title>Seguridad en trabajos confinados</title>
...
<body>
<h1>Videotest de Seguridad en trabajos confinados</h1>

<div class="modal-background">
<div class="container">

<!-- Verificación si el Videotest ha sido completado -->
<?php if ($_SESSION['videotest_completado_curso_seguridadTC'][26]): ?>
    <?php if ($_SESSION['estado_final_curso_seguridadTC'][26] == 'APROBADO'): ?>
        <!-- Mensaje de éxito si se aprueba el examen -->
        <div class="mensaje">
            Felicidades! Ya has aprobado el examen con una nota de <?php echo $_SESSION['nota_final_curso_seguridadTC'][26]; ?> puntos.
    ...
    <?php else: ?>
        ...
        <?php if ($_SESSION['intentos_curso_seguridadTC'][26] < 3): ?>
            <div class="mensaje">
                Has reprobado el examen con <?php echo $_SESSION['nota_final']; ?> puntos. Te queda 
                <?php echo 3 - $_SESSION['intentos_curso_seguridadTC'][26]; ?> intento(s). Puedes intentar nuevamente.
            </div>
            <div class="mensaje">
                El puntaje más alto obtenido hasta ahora es <?php echo max($_SESSION['notas_curso_seguridadTC']); ?> puntos.
            </div>
    ...
    <div class="modal-content">
        ...
        <form method="post" id="iniciarExamenForm">
            <div class="button-container">
                ...
                <button type="button" class="modal-button" onclick="window.location.href='seguridadTC.php '">Cancelar</button>
            </div>
        </form>
    </div>
</div>
