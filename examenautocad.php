...
function inicializarSesion() {
    if (!isset($_SESSION['intentos_por_curso_autocad'])) {
        $_SESSION['intentos_por_curso_autocad'] = [];
    }
    
    // ID 2 corresponde al curso de primeros auxilios
    if (!isset($_SESSION['intentos_por_curso_autocad'][24])) {
        $_SESSION['intentos_por_curso_autocad'][24] = 0;
    }
    
    if (!isset($_SESSION['notas_por_curso_autocad'])) {
        $_SESSION['notas_por_curso_autocad'] = [];
    }
    if (!isset($_SESSION['notas_por_curso_autocad'][24])) {
        $_SESSION['notas_por_curso_autocad'][24] = [];
    }
    if (!isset($_SESSION['examen_completado_por_curso_autocad'][24])) {
        $_SESSION['examen_completado_por_curso_autocad'][24] = false;
    }    
    ...
    $_SESSION['examen_completado_por_curso_autocad'][24] = false;
    $_SESSION['nota_final_por_curso_autocad'][24] = 0;
    $_SESSION['estado_final_por_curso_autocad'][24] = '';
    $_SESSION['nota_mas_alta_por_curso_autocad'][24] = 0;
    $_SESSION['id_curso'] = 24; // Curso de primeros auxilios
}

// Inicializar sesión si no existe o si se solicita reiniciar
if (!isset($_SESSION['intentos_por_curso_autocad']) || isset($_POST['reiniciar'])) {
    inicializarSesion();
}
...
if (isset($_POST['iniciar_examen'])) {
    if ($_SESSION['intentos_por_curso_autocad'][24] < 3) {
        $_SESSION['examen_iniciado'] = true;
        $_SESSION['tiempo_inicio'] = time();
        $_SESSION['tiempo_restante'] = $duracion_examen;
        header("Location: examenautocad.php");
        exit();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['respuestas'])) {
    if (tiempoExpirado()) {
        $_SESSION['notas_por_curso_autocad'][24][] = 0;
        ...
        $_SESSION['notas_por_curso_autocad'][24][] = $nota;
        $_SESSION['examen_iniciado'] = false;
        ...
        if ($nota > $_SESSION['nota_mas_alta_por_curso_autocad'][24]) {
            $_SESSION['nota_mas_alta_por_curso_autocad'][24] = $nota;
        }

        // Incrementar intento actual en sesión
        $_SESSION['intentos_por_curso_autocad'][24]++;
        $intento_actual = $_SESSION['intentos_por_curso_autocad'][24];

        // Actualizar base de datos
        if (isset($_SESSION['nota_mas_alta_por_curso_autocad'][24]) && isset($_SESSION['DNI'])) {
            ...
            $DNI = $_SESSION['DNI'];
            $nota_mas_alta = $_SESSION['nota_mas_alta_por_curso_autocad'][24];
        ...
        if ($intento_actual < 3) {
            $mensaje = "Has completado el intento $intento_actual con $nota puntos. Estado: " . $_SESSION['estado_final_por_curso_autocad'][24];
        } else {
            $mensaje = "En este último intento tu nota es $nota. La nota final de tus 3 intentos ha sido " . $_SESSION['nota_mas_alta_por_curso_autocad'][24];
            $_SESSION['examen_completado_por_curso_autocad'][24] = true;
        }
    }
}
...
if (isset($_POST['reiniciar'])) {
    ...
    $sql = "UPDATE inscripciones SET `intentose1` = 0, `intentose2` = 0, `intentose3` = 0, `examen_final` = 0 WHERE DNI = ? AND id_curso = 24";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $DNI);

    if ($stmt->execute()) {
        $_SESSION['nota_mas_alta_por_curso_autocad'][24] = 0;
        $_SESSION['intentos_por_curso_autocadE'][24] = 0;
        $_SESSION['notas_por_curso_autocad'][24] = [];
        $_SESSION['examen_completado_por_curso_autocad'][24] = false;
        $_SESSION['estado_final_por_curso_autocad'][24] = '';
...
<body>
    ...
    <div class="container">
        <h1>Examen de AUTOCAD 2D Y 3D</h1>
    <?php if ($_SESSION['examen_completado_por_curso_autocad'][24]): ?>
    <?php if ($_SESSION['estado_final_por_curso_autocad'][24] == 'APROBADO'): ?>
    ...
    <?php else: ?>
        ...
        <?php if ($_SESSION['intentos_por_curso_autocad'][24] < 3): ?>
            <div class="mensaje">
                Has reprobado el examen con <?php echo $_SESSION['nota_final']; ?> puntos. Te queda 
                <?php echo 3 - $_SESSION['intentos_por_curso_autocad'][24]; ?> intento(s). Puedes intentar nuevamente.
            </div>
            <div class="mensaje">
                El puntaje más alto obtenido hasta ahora es <?php echo max($_SESSION['notas_por_curso_autocad']); ?> puntos.
            </div>
...
<?php elseif ($_SESSION['intentos_por_curso_autocad'][24] < 3): ?>
...
<div id="inicioModal" class="modal">
    <div class="modal-content">
                ...
                <div class="button-container">
                        ...
                        <button type="button" class="modal-button" onclick="window.location.href='autocad.php'">Volver atrás</button>
            </div>
        </form>
    </div>
</div>
