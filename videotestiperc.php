...
function inicializarSesion() {
    if (!isset($_SESSION['intentos_curso_iperc'])) {
        $_SESSION['intentos_curso_iperc'] = [];
    }
    
    // ID 2 corresponde al curso de IPERC
    if (!isset($_SESSION['intentos_curso_iperc'][4])) {
        $_SESSION['intentos_curso_iperc'][4] = 0;
    }
    
    if (!isset($_SESSION['notas_curso_iperc'])) {
        $_SESSION['notas_curso_iperc'] = [];
    }
    if (!isset($_SESSION['notas_curso_iperc'][4])) {
        $_SESSION['notas_curso_iperc'][4] = [];
    }
    if (!isset($_SESSION['videotest_completado_curso_iperc'][4])) {
        $_SESSION['videotest_completado_curso_iperc'][4] = false;
    }    
    ...
    $_SESSION['videotest_completado_curso_iperc'][4] = false;
    $_SESSION['nota_final_curso_iperc'][4] = 0;
    $_SESSION['estado_final_curso_iperc'][4] = '';
    $_SESSION['nota_mas_alta_curso_iperc'][4] = 0;
    $_SESSION['id_curso'] = 4; // Curso de Uso y manejo de Extintores
}

// Inicializar sesión si no existe o si se solicita reiniciar
if (!isset($_SESSION['intentos_curso_iperc']) || isset($_POST['reiniciar'])) {
    inicializarSesion();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['respuestas'])) {
    if (tiempoExpirado()) {
        $_SESSION['notas_curso_iperc'][4][] = 0;
        $_SESSION['estado_final_curso_iperc'][4] = 'REPROBADO';
        ...
        $_SESSION['notas_curso_iperc'][4][] = $nota;
        $_SESSION['videotest_iniciado'] = false;

        // Evaluar estado
        if ($nota >= 13) {
            $_SESSION['estado_final_curso_iperc'][4] = 'APROBADO';
            $_SESSION['nota_final_curso_iperc'][4] = max($_SESSION['notas_curso_iperc'][4]);
            $_SESSION['videotest_completado_curso_iperc'][4] = true;
        } else {
            $_SESSION['estado_final_curso_iperc'][4] = 'REPROBADO';
        }

        // Actualizar la mejor nota
        if ($nota > $_SESSION['nota_mas_alta_curso_iperc'][4]) {
            $_SESSION['nota_mas_alta_curso_iperc'][4] = $nota;
        }

        // Incrementar intento actual en sesión
        $_SESSION['intentos_curso_iperc'][4]++;
        $intento_actual = $_SESSION['intentos_curso_iperc'][4];

        // Actualizar base de datos
        if (isset($_SESSION['nota_mas_alta_curso_iperc'][4]) && isset($_SESSION['DNI'])) {
            ...
            $DNI = $_SESSION['DNI'];
            $nota_mas_alta = $_SESSION['nota_mas_alta_curso_iperc'][4];

            // Verificar inscripción
            $sqlVerificar = "SELECT id_curso FROM inscripciones WHERE DNI = ? AND id_curso = 4";
        ...
        if ($intento_actual < 3) {
            $mensaje = "Has completado el intento $intento_actual con $nota puntos. Estado: " . $_SESSION['estado_final_curso_iperc'][4];
        } else {
            $mensaje = "En este último intento tu nota es $nota. La nota final de tus 3 intentos ha sido " . $_SESSION['nota_mas_alta_curso_iperc'][4];
            $_SESSION['videotest_completado_curso_iperc'][4] = true;
        }
    }
}
...
if (isset($_POST['reiniciar'])) {
    ...
    $sql = "UPDATE inscripciones SET `intento1` = 0, `intento2` = 0, `intento3` = 0, `nota_videotest` = 0 WHERE DNI = ? AND id_curso = 4";
    $stmt = $conn->prepare($sql);
...
<body>
<h1>Videotest de IPERC</h1>

<div class="modal-background">
<div class="container">

<!-- Verificación si el Videotest ha sido completado -->
<?php if ($_SESSION['videotest_completado_curso_iperc'][4]): ?>
    <?php if ($_SESSION['estado_final_curso_iperc'][4] == 'APROBADO'): ?>
        <!-- Mensaje de éxito si se aprueba el examen -->
        <div class="mensaje">
            Felicidades! Ya has aprobado el examen con una nota de <?php echo $_SESSION['nota_final_curso_iperc'][4]; ?> puntos.
            No necesitas realizar más intentos.
        </div>
        <!-- Botón para volver al curso -->
        <form method="post">
            <button type="button" value="Volver a mi curso" class="volver" onclick="window.location.href='iperc.php'">Volver a mi curso</button>
        </form>

    <?php else: ?>
        <!-- Si no está aprobado, y se tiene menos de 3 intentos -->
        <?php if ($_SESSION['intentos_curso_iperc'][4] < 3): ?>
            <div class="mensaje">
                Has reprobado el examen con <?php echo $_SESSION['nota_final']; ?> puntos. Te queda 
                <?php echo 3 - $_SESSION['intentos_curso_iperc'][4]; ?> intento(s). Puedes intentar nuevamente.
            </div>
            <div class="mensaje">
                El puntaje más alto obtenido hasta ahora es <?php echo max($_SESSION['notas_curso_iperc']); ?> puntos.
            </div>
        ...
        <?php else: ?>
            ...
            <form method="post">
                <input type="button" value="Volver a mi curso" class="volver" onclick="window.location.href='iperc.php'">
            </form>
            <div class="mensaje">
    ...
    <div class="modal-content">
        ...
        <form method="post" id="iniciarExamenForm">
            <div class="button-container">
                ...
                <button type="button" class="modal-button" onclick="window.location.href='iperc.php '">Cancelar</button>
            </div>
        </form>
    </div>
</div>
