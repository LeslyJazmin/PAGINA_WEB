...
function inicializarSesion() {
    ...
    if (!isset($_SESSION['notas_por_curso_Homologacion6G'][30])) {
        $_SESSION['notas_por_curso_Homologacion6G'][30] = [];
    }
...
if (!isset($_SESSION['intentos_por_curso_Homologacion6G']) || isset($_POST['reiniciar'])) {
    inicializarSesion();
}
...
if (isset($_POST['iniciar_examen'])) {
    if ($_SESSION['intentos_por_curso_Homologacion6G'][30] < 3) {
        $_SESSION['examen_iniciado'] = true;
        $_SESSION['tiempo_inicio'] = time();
        $_SESSION['tiempo_restante'] = $duracion_examen;
        header("Location: examenHomologacion6G.php");
        exit();
    }
}
...
$preguntas = [
    ['¿Qué tipo de corriente es más recomendable para soldadura SMAW con electrodo E7018 en 6G?', 
    ['Corriente alterna (AC)', 'Corriente continua (DC+)', 'Corriente continua (DC-)'], 
    1], // Respuesta correcta: "Corriente continua (DC+)" (índice 1)

   ['¿Qué defecto común puede ocurrir si la velocidad de desplazamiento es muy rápida en 6G?', 
    ['Falta de fusión', 'Exceso de refuerzo', 'Inclusiones de escoria'], 
    0], // Respuesta correcta: "Falta de fusión" (índice 0)

   ['¿Qué prueba no destructiva se usa para inspeccionar soldaduras en homologación 6G?', 
    ['Radiografía industrial', 'Ensayo de impacto', 'Doblado en raíz'], 
    0], // Respuesta correcta: "Radiografía industrial" (índice 0)

   ['¿Cuál es el ángulo recomendado del electrodo para una soldadura en posición 6G?', 
    ['75-85°', '45-60°', '90°'], 
    0], // Respuesta correcta: "75-85°" (índice 0)

   ['¿Qué técnica ayuda a reducir la acumulación de material fundido al soldar en 6G?', 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['respuestas'])) {
    if (tiempoExpirado()) {
        $_SESSION['notas_por_curso_Homologacion6G'][30][] = 0;
        $_SESSION['estado_final_por_curso_Homologacion6G'][30] = 'REPROBADO';
        ...
        $_SESSION['notas_por_curso_Homologacion6G'][30][] = $nota;
        $_SESSION['examen_iniciado'] = false;

        // Evaluar estado
        if ($nota >= 13) {
            $_SESSION['estado_final_por_curso_Homologacion6G'][30] = 'APROBADO';
            $_SESSION['nota_final_por_curso_Homologacion6G'][30] = $_SESSION['nota_final_por_curso_Homologacion6G'][30];
            $_SESSION['examen_completado_por_curso_Homologacion6G'][30] = true;
        } else {
            $_SESSION['estado_final_por_curso_Homologacion6G'][30] = 'REPROBADO';
        }

        // Actualizar la mejor nota
        if ($nota > $_SESSION['nota_mas_alta_por_curso_Homologacion6G'][30]) {
            $_SESSION['nota_mas_alta_por_curso_Homologacion6G'][30] = $nota;
        }

        // Incrementar intento actual en sesión
        $_SESSION['intentos_por_curso_Homologacion6G'][30]++;
        $intento_actual = $_SESSION['intentos_por_curso_Homologacion6G'][30];

        // Actualizar base de datos
        if (isset($_SESSION['nota_mas_alta_por_curso_Homologacion6G'][30]) && isset($_SESSION['DNI'])) {
            ...
            $DNI = $_SESSION['DNI'];
            $nota_mas_alta = $_SESSION['nota_mas_alta_por_curso_Homologacion6G'][30];
        ...
        if ($intento_actual < 3) {
            $mensaje = "Has completado el intento $intento_actual con $nota puntos. Estado: " . $_SESSION['estado_final_por_curso_Homologacion6G'][30];
        } else {
            $mensaje = "En este último intento tu nota es $nota. La nota final de tus 3 intentos ha sido " . $_SESSION['nota_mas_alta_por_curso_Homologacion6G'][30];
            $_SESSION['examen_completado_por_curso_Homologacion6G'][30] = true;
        }
    }
}
...
<head>
    ...
    <title>Examen de Homologación 6G en proceso smaw</title>
