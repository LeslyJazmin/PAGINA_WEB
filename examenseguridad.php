...
function inicializarSesion() {
    if (!isset($_SESSION['intentos_por_curso_seguridad'])) {
        $_SESSION['intentos_por_curso_seguridad'] = [];
    }

    // ID 19 corresponde al curso de Seguridad para Trabajos con Alturas
    if (!isset($_SESSION['intentos_por_curso_seguridad'][19])) {
        $_SESSION['intentos_por_curso_seguridad'][19] = 0;
    }

    if (!isset($_SESSION['notas_por_curso_seguridad'])) {
        $_SESSION['notas_por_curso_seguridad'] = [];
    }
    if (!isset($_SESSION['notas_por_curso_seguridad'][19])) {
        $_SESSION['notas_por_curso_seguridad'][19] = [];
    }
    if (!isset($_SESSION['examen_completado_por_curso_seguridad'][19])) {
        $_SESSION['examen_completado_por_curso_seguridad'][19] = false;
    }
    ...
    $_SESSION['examen_completado_por_curso_seguridad'][19] = false;
    $_SESSION['nota_final_por_curso_seguridad'][19] = 0;
    $_SESSION['estado_final_por_curso_seguridad'][19] = '';
    $_SESSION['nota_mas_alta_por_curso_seguridad'][19] = 0;
    $_SESSION['id_curso'] = 19; // Curso de Seguridad para Trabajos con Alturas
}

// Inicializar sesión si no existe o si se solicita reiniciar
if (!isset($_SESSION['intentos_por_curso_seguridad']) || isset($_POST['reiniciar'])) {
    inicializarSesion();
}
...
if (isset($_POST['iniciar_examen'])) {
    if ($_SESSION['intentos_por_curso_seguridad'][19] < 3) {
        $_SESSION['examen_iniciado'] = true;
        $_SESSION['tiempo_inicio'] = time();
        $_SESSION['tiempo_restante'] = $duracion_examen;
        header("Location: examenseguridad.php");
        exit();
    }
}
...
// Obtener las preguntas del examen para el curso de Seguridad para Trabajos con Alturas (id_curso = 19)
$preguntas = obtenerPreguntas(19);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['respuestas'])) {
    if (tiempoExpirado()) {
        $_SESSION['notas_por_curso_seguridad'][19][] = 0;
        ...
        $_SESSION['notas_por_curso_seguridad'][19][] = $nota;
        $_SESSION['examen_iniciado'] = false;
        ...
        if ($nota > $_SESSION['nota_mas_alta_por_curso_seguridad'][19]) {
            $_SESSION['nota_mas_alta_por_curso_seguridad'][19] = $nota;
        }

        // Incrementar intento actual en sesión
        $_SESSION['intentos_por_curso_seguridad'][19]++;
        $intento_actual = $_SESSION['intentos_por_curso_seguridad'][19];

        // Actualizar base de datos
        if (isset($_SESSION['nota_mas_alta_por_curso_seguridad'][19]) && isset($_SESSION['DNI'])) {
            $conn = new mysqli('localhost', 'root', '', 'usuario');
            if ($conn->connect_error) {
                die("Error de conexión: " . $conn->connect_error);
            }
            ...
            $DNI = $_SESSION['DNI'];
            $nota_mas_alta = $_SESSION['nota_mas_alta_por_curso_seguridad'][19];
        ...
        if ($intento_actual < 3) {
            $mensaje = "Has completado el intento $intento_actual con $nota puntos. Estado: " . $_SESSION['estado_final_por_curso_seguridad'][19];
        } else {
            $mensaje = "En este último intento tu nota es $nota. La nota final de tus 3 intentos ha sido " . $_SESSION['nota_mas_alta_por_curso_seguridad'][19];
            $_SESSION['examen_completado_por_curso_seguridad'][19] = true;
        }
    }
}
...
if (isset($_POST['reiniciar'])) {
    ...
    $sql = "UPDATE inscripciones SET `intentose1` = 0, `intentose2` = 0, `intentose3` = 0, `examen_final` = 0 WHERE DNI = ? AND id_curso = 19";
    $stmt = $conn->prepare($sql);
...
<head>
    ...
    <title>Examen de Seguridad para Trabajos con Alturas</title>
...
<body>
    ...
    <div class="container">
        <h1>Examen de Seguridad para Trabajos con Alturas</h1>
    <?php if ($_SESSION['examen_completado_por_curso_seguridad'][19]): ?>
    <?php if ($_SESSION['estado_final_por_curso_seguridad'][19] == 'APROBADO'): ?>
            <div class="mensaje">
    ...
    <?php else: ?>
            <?php if ($_SESSION['intentos_por_curso_seguridad'][19] < 3): ?>
                <div class="mensaje">
                    Has reprobado el examen con <?php echo $_SESSION['nota_final']; ?> puntos. Te queda
                    <?php echo 3 - $_SESSION['intentos_por_curso_seguridad'][19]; ?> intento(s). Puedes intentar nuevamente.
                </div>
                <div class="mensaje">
                    El puntaje más alto obtenido hasta ahora es <?php echo max($_SESSION['notas_por_curso_seguridad'][19]); ?> puntos.
                </div>
...
<?php elseif ($_SESSION['intentos_por_curso_seguridad'][19] < 3): ?>
...
<div id="inicioModal" class="modal">
    <div class="modal-content">
        ...
        <div class="button-container">
            ...
            <button type="button" class="modal-button" onclick="window.location.href='seguridadTA.php'">Volver atrás</button>
        </div>
        </form>
    </div>
</div>
