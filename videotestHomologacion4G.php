...
function inicializarSesion() {
    if (!isset($_SESSION['intentos_curso_Homologacion4G'])) {
        $_SESSION['intentos_curso_Homologacion4G'] = [];
    }
    
    // ID 2 corresponde al curso de primeros auxilios
    if (!isset($_SESSION['intentos_curso_Homologacion4G'][29])) {
        $_SESSION['intentos_curso_Homologacion4G'][29] = 0;
    }
    
    if (!isset($_SESSION['notas_curso_Homologacion4G'])) {
        $_SESSION['notas_curso_Homologacion4G'] = [];
    }
    if (!isset($_SESSION['notas_curso_Homologacion4G'][29])) {
        $_SESSION['notas_curso_Homologacion4G'][29] = [];
    }
    if (!isset($_SESSION['videotest_completado_curso_Homologacion4G'][29])) {
        $_SESSION['videotest_completado_curso_Homologacion4G'][29] = false;
    }    
    ...
    $_SESSION['videotest_completado_curso_Homologacion4G'][29] = false;
    $_SESSION['nota_final_curso_Homologacion4G'][29] = 0;
    $_SESSION['estado_final_curso_Homologacion4G'][29] = '';
    $_SESSION['nota_mas_alta_curso_Homologacion4G'][29] = 0;
    $_SESSION['id_curso'] = 29; // Curso de Uso y manejo de Extintores
}

// Inicializar sesión si no existe o si se solicita reiniciar
if (!isset($_SESSION['intentos_curso_Homologacion4G']) || isset($_POST['reiniciar'])) {
    inicializarSesion();
}
...
if (isset($_POST['iniciar_videotest'])) {
    if ($_SESSION['intentos_curso_Homologacion4G'][29] < 3) {
        $_SESSION['videotest_iniciado'] = true;
        $_SESSION['tiempo_inicio'] = time();
        $_SESSION['tiempo_restante'] = $duracion_examen;
        header("Location: videotestHomologacion4G.php");
        exit();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['respuestas'])) {
    if (tiempoExpirado()) {
        $_SESSION['notas_curso_Homologacion4G'][28][] = 0;
        $_SESSION['estado_final_curso_Homologacion4G'][28] = 'REPROBADO';
        ...
        $_SESSION['notas_curso_Homologacion4G'][29][] = $nota;
        $_SESSION['videotest_iniciado'] = false;

        // Evaluar estado
        if ($nota >= 13) {
            $_SESSION['estado_final_curso_Homologacion4G'][29] = 'APROBADO';
            $_SESSION['nota_final_curso_Homologacion4G'][29] = max($_SESSION['notas_curso_Homologacion4G'][29]);
            $_SESSION['videotest_completado_curso_Homologacion4G'][29] = true;
        } else {
            $_SESSION['estado_final_curso_Homologacion4G'][29] = 'REPROBADO';
        }

        // Actualizar la mejor nota
        if ($nota > $_SESSION['nota_mas_alta_curso_Homologacion4G'][29]) {
            $_SESSION['nota_mas_alta_curso_Homologacion4G'][29] = $nota;
        }

        // Incrementar intento actual en sesión
        $_SESSION['intentos_curso_Homologacion4G'][29]++;
        $intento_actual = $_SESSION['intentos_curso_Homologacion4G'][29];

        // Actualizar base de datos
        if (isset($_SESSION['nota_mas_alta_curso_Homologacion4G'][29]) && isset($_SESSION['DNI'])) {
            ...
            $DNI = $_SESSION['DNI'];
            $nota_mas_alta = $_SESSION['nota_mas_alta_curso_Homologacion4G'][29];
        ...
        if ($intento_actual < 3) {
            $mensaje = "Has completado el intento $intento_actual con $nota puntos. Estado: " . $_SESSION['estado_final_curso_Homologacion4G'][29];
        } else {
            $mensaje = "En este último intento tu nota es $nota. La nota final de tus 3 intentos ha sido " . $_SESSION['nota_mas_alta_curso_Homologacion4G'][29];
            $_SESSION['videotest_completado_curso_Homologacion4G'][29] = true;
        }
    }
}
...
if (isset($_POST['reiniciar'])) {
    ...
    $sql = "UPDATE inscripciones SET `intento1` = 0, `intento2` = 0, `intento3` = 0, `nota_videotest` = 0 WHERE DNI = ? AND id_curso = 29";
    $stmt = $conn->prepare($sql);
...
<head>
    ...
    <title>Homologación 4G en proceso smaw</title>
...
<body>
<h1>Videotest de Homologación 4G en proceso smaw</h1>

<div class="modal-background">
<div class="container">

<!-- Verificación si el Videotest ha sido completado -->
<?php if ($_SESSION['videotest_completado_curso_Homologacion4G'][29]): ?>
    <?php if ($_SESSION['estado_final_curso_Homologacion4G'][29] == 'APROBADO'): ?>
        <!-- Mensaje de éxito si se aprueba el examen -->
        <div class="mensaje">
            Felicidades! Ya has aprobado el examen con una nota de <?php echo $_SESSION['nota_final_curso_Homologacion4G'][29]; ?> puntos.
    ...
    <?php else: ?>
        ...
        <?php if ($_SESSION['intentos_curso_Homologacion4G'][29] < 3): ?>
            <div class="mensaje">
                Has reprobado el examen con <?php echo $_SESSION['nota_final']; ?> puntos. Te queda 
                <?php echo 3 - $_SESSION['intentos_curso_Homologacion4G'][29]; ?> intento(s). Puedes intentar nuevamente.
            </div>
            <div class="mensaje">
                El puntaje más alto obtenido hasta ahora es <?php echo max($_SESSION['notas_curso_Homologacion4G']); ?> puntos.
            </div>
    ...
    <div class="modal-content">
        ...
        <form method="post" id="iniciarExamenForm">
            <div class="button-container">
                ...
                <button type="button" class="modal-button" onclick="window.location.href='Homologacion4G.php '">Cancelar</button>
            </div>
        </form>
    </div>
</div>
