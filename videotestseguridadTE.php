...
function inicializarSesion() {
    if (!isset($_SESSION['intentos_curso_seguridadTE'])) {
        $_SESSION['intentos_curso_seguridadTE'] = [];
    }
    
    // ID 2 corresponde al curso de primeros auxilios
    if (!isset($_SESSION['intentos_curso_seguridadTE'][22])) {
        $_SESSION['intentos_curso_seguridadTE'][22] = 0;
    }
    
    if (!isset($_SESSION['notas_curso_seguridadTE'])) {
        $_SESSION['notas_curso_seguridadTE'] = [];
    }
    if (!isset($_SESSION['notas_curso_seguridadTE'][22])) {
        $_SESSION['notas_curso_seguridadTE'][22] = [];
    }
    if (!isset($_SESSION['videotest_completado_curso_seguridadTE'][22])) {
        $_SESSION['videotest_completado_curso_seguridadTE'][22] = false;
    }    
    ...
    $_SESSION['videotest_completado_curso_seguridadTE'][22] = false;
    $_SESSION['nota_final_curso_seguridadTE'][22] = 0;
    $_SESSION['estado_final_curso_seguridadTE'][22] = '';
    $_SESSION['nota_mas_alta_curso_seguridadTE'][22] = 0;
    $_SESSION['id_curso'] = 22; // Curso de Uso y manejo de Extintores
}

// Inicializar sesión si no existe o si se solicita reiniciar
if (!isset($_SESSION['intentos_curso_seguridadTE']) || isset($_POST['reiniciar'])) {
    inicializarSesion();
}
...
if (isset($_POST['iniciar_videotest'])) {
    if ($_SESSION['intentos_curso_seguridadTE'][22] < 3) {
        $_SESSION['videotest_iniciado'] = true;
        $_SESSION['tiempo_inicio'] = time();
        $_SESSION['tiempo_restante'] = $duracion_examen;
        header("Location: videotestseguridadTE.php");
        exit();
    }
}
...
$sql_preguntas = "SELECT id, pregunta FROM preguntas WHERE id_curso = 22";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['respuestas'])) {
    if (tiempoExpirado()) {
        $_SESSION['notas_curso_seguridadTE'][22][] = 0;
        $_SESSION['estado_final_curso_seguridadTE'][22] = 'REPROBADO';
        ...
        $_SESSION['notas_curso_seguridadTE'][22][] = $nota;
        $_SESSION['videotest_iniciado'] = false;

        // Evaluar estado
        if ($nota >= 13) {
            $_SESSION['estado_final_curso_seguridadTE'][22] = 'APROBADO';
            $_SESSION['nota_final_curso_seguridadTE'][22] = max($_SESSION['notas_curso_seguridadTE'][22]);
            $_SESSION['videotest_completado_curso_seguridadTE'][22] = true;
        } else {
            $_SESSION['estado_final_curso_seguridadTE'][22] = 'REPROBADO';
        }

        // Actualizar la mejor nota
        if ($nota > $_SESSION['nota_mas_alta_curso_seguridadTE'][22]) {
            $_SESSION['nota_mas_alta_curso_seguridadTE'][22] = $nota;
        }

        // Incrementar intento actual en sesión
        $_SESSION['intentos_curso_seguridadTE'][22]++;
        $intento_actual = $_SESSION['intentos_curso_seguridadTE'][22];

        // Actualizar base de datos
        if (isset($_SESSION['nota_mas_alta_curso_seguridadTE'][22]) && isset($_SESSION['DNI'])) {
            $conn = new mysqli('localhost', 'root', '', 'usuario');
            if ($conn->connect_error) {
                die("Error de conexión: " . $conn->connect_error);
            }
            ...
            $DNI = $_SESSION['DNI'];
            $nota_mas_alta = $_SESSION['nota_mas_alta_curso_seguridadTE'][22];

            // Verificar inscripción
            $sqlVerificar = "SELECT id_curso FROM inscripciones WHERE DNI = ? AND id_curso = 22";
        ...
        if ($intento_actual < 3) {
            $mensaje = "Has completado el intento $intento_actual con $nota puntos. Estado: " . $_SESSION['estado_final_curso_seguridadTE'][22];
        } else {
            $mensaje = "En este último intento tu nota es $nota. La nota final de tus 3 intentos ha sido " . $_SESSION['nota_mas_alta_curso_seguridadTE'][22];
            $_SESSION['videotest_completado_curso_seguridadTE'][22] = true;
        }
    }
}
...
if (isset($_POST['reiniciar'])) {
    ...
    $sql = "UPDATE inscripciones SET `intento1` = 0, `intento2` = 0, `intento3` = 0, `nota_videotest` = 0 WHERE DNI = ? AND id_curso = 22";
    $stmt = $conn->prepare($sql);
...
<head>
    ...
    <title>Seguridad para Trabajos en Zanjas o Excavaciones</title>
...
<body>
<h1>Videotest de Seguridad para Trabajos en Zanjas o Excavaciones</h1>

<div class="modal-background">
<div class="container">

<!-- Verificación si el Videotest ha sido completado -->
<?php if ($_SESSION['videotest_completado_curso_seguridadTE'][22]): ?>
    <?php if ($_SESSION['estado_final_curso_seguridadTE'][22] == 'APROBADO'): ?>
        <!-- Mensaje de éxito si se aprueba el examen -->
        <div class="mensaje">
            Felicidades! Ya has aprobado el examen con una nota de <?php echo $_SESSION['nota_final_curso_seguridadTE'][22]; ?> puntos.
            No necesitas realizar más intentos.
        </div>
        <!-- Botón para volver al curso -->
        <form method="post">
            <button type="button" value="Volver a mi curso" class="volver" onclick="window.location.href='seguridadTE.php'">Volver a mi curso</button>
        </form>

    <?php else: ?>
        <!-- Si no está aprobado, y se tiene menos de 3 intentos -->
        <?php if ($_SESSION['intentos_curso_seguridadTE'][22] < 3): ?>
            <div class="mensaje">
                Has reprobado el examen con <?php echo $_SESSION['nota_final']; ?> puntos. Te queda 
                <?php echo 3 - $_SESSION['intentos_curso_seguridadTE'][22]; ?> intento(s). Puedes intentar nuevamente.
            </div>
            <div class="mensaje">
                El puntaje más alto obtenido hasta ahora es <?php echo max($_SESSION['notas_curso_seguridadTE']); ?> puntos.
            </div>
        ...
        <?php else: ?>
            ...
            <form method="post">
                <input type="button" value="Volver a mi curso" class="volver" onclick="window.location.href='seguridadTE.php'">
            </form>
            <div class="mensaje">
...
<?php elseif ($_SESSION['intentos_curso_seguridadTE'][22] < 3): ?>
    <?php if (!$_SESSION['videotest_iniciado']): ?>
        ...
        <?php endif; ?>
        <input type="button" value="Cerrar y salir" class="modal-button" onclick="window.location.href='seguridadTE.php'">
    <?php else: ?>
    ...
    <div class="modal-content">
        ...
        <form method="post" id="iniciarExamenForm">
            <div class="button-container">
                ...
                <button type="button" class="modal-button" onclick="window.location.href='seguridadTE.php '">Cancelar</button>
            </div>
        </form>
    </div>
</div>
...
<div id="modalSinPreguntas" class="modal-overlay" style="display: <?= $mostrar_modal ? 'flex' : 'none' ?>;">
    <div class="modal-content">
        ...
        <button onclick="window.location.href='seguridadTE.php'">Cerrar</button>
    </div>
</div>
</body>
</html>    
