...
function inicializarSesion() {
    if (!isset($_SESSION['intentos_por_curso_me'])) {
        $_SESSION['intentos_por_curso_me'] = [];
    }
    
    // ID 2 corresponde al curso de primeros auxilios
    if (!isset($_SESSION['intentos_por_curso_me'][3])) {
        $_SESSION['intentos_por_curso_me'][3] = 0;
    }
    
    if (!isset($_SESSION['notas_por_curso_me'])) {
        $_SESSION['notas_por_curso_me'] = [];
    }
    if (!isset($_SESSION['notas_por_curso_me'][3])) {
        $_SESSION['notas_por_curso_me'][3] = [];
    }
    if (!isset($_SESSION['examen_completado_por_curso_me'][3])) {
        $_SESSION['examen_completado_por_curso_me'][3] = false;
    }   
    
    $_SESSION['tiempo_inicio'] = 0;
    $_SESSION['tiempo_restante'] = 20 * 60;  // 20 minutos en segundos
    $_SESSION['examen_iniciado'] = false;
    $_SESSION['examen_completado_por_curso_me'][3] = false;
    $_SESSION['nota_final_por_curso_me'][3] = 0;
    $_SESSION['estado_final_por_curso_me'][3] = '';
    $_SESSION['nota_mas_alta_por_curso_me'][3] = 0;
    $_SESSION['id_curso'] = 3; // Curso de primeros auxilios
}

// Inicializar sesión si no existe o si se solicita reiniciar
if (!isset($_SESSION['intentos_por_curso_me']) || isset($_POST['reiniciar'])) {
    inicializarSesion();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['respuestas'])) {
    if (tiempoExpirado()) {
        $_SESSION['notas_por_curso_me'][3][] = 0;
        $_SESSION['estado_final_por_curso_me'][3] = 'REPROBADO';
        ...
        $_SESSION['notas_por_curso_me'][3][] = $nota;
        $_SESSION['examen_iniciado'] = false;

        // Evaluar estado
        if ($nota >= 13) {
            $_SESSION['estado_final_por_curso_me'][3] = 'APROBADO';
            $_SESSION['nota_final_por_curso_me'][3] = max($_SESSION['nota_final_por_curso_me'][3]);
            $_SESSION['examen_completado_por_curso_me'][3] = true;
        } else {
            $_SESSION['estado_final_por_curso_me'][3] = 'REPROBADO';
        }

        // Actualizar la mejor nota
        if ($nota > $_SESSION['nota_mas_alta_por_curso_me'][3]) {
            $_SESSION['nota_mas_alta_por_curso_me'][3] = $nota;
        }

        // Incrementar intento actual en sesión
        $_SESSION['intentos_por_curso_me'][3]++;
        $intento_actual = $_SESSION['intentos_por_curso_me'][3];

        // Actualizar base de datos
        if (isset($_SESSION['nota_mas_alta_por_curso_me'][3]) && isset($_SESSION['DNI'])) {
            $conn = new mysqli('localhost', 'root', '', 'usuario');
            if ($conn->connect_error) {
                die("Error de conexión: " . $conn->connect_error);
            }
            ...
            $DNI = $_SESSION['DNI'];
            $nota_mas_alta = $_SESSION['nota_mas_alta_por_curso_me'][3];
        ...
        if ($intento_actual < 3) {
            $mensaje = "Has completado el intento $intento_actual con $nota puntos. Estado: " . $_SESSION['estado_final_por_curso_me'][3];
        } else {
            $mensaje = "En este último intento tu nota es $nota. La nota final de tus 3 intentos ha sido " . $_SESSION['nota_mas_alta_por_curso_me'][3];
            $_SESSION['examen_completado_por_curso_me'][3] = true;
        }
    }
...
if (isset($_POST['reiniciar'])) {
    ...
    $sql = "UPDATE inscripciones SET `intentose1` = 0, `intentose2` = 0, `intentose3` = 0, `examen_final` = 0 WHERE DNI = ? AND id_curso = 3";
    $stmt = $conn->prepare($sql);
...
<head>
    ...
    <title>Examen de Uso y manejo de Extintores</title ```html
...
input.volver {
    ...
    margin-top: 99px; /* Mueve el botón hacia arriba */
}
...
<body>
    <div class="container">
        <h1>Examen de Uso y manejo de Extintores</h1>
        
        <?php if ($_SESSION['examen_completado_por_curso_me'][3]): ?>
    <?php if ($_SESSION['estado_final_por_curso_me'][3] == 'APROBADO'): ?>
    ...
    <?php else: ?>
        ...
        <?php if ($_SESSION['intentos_por_curso_me'][3] < 3): ?>
            <div class="mensaje">
                Has reprobado el examen con <?php echo $_SESSION['nota_final']; ?> puntos. Te queda 
                <?php echo 3 - $_SESSION['intentos_por_curso_me'][3]; ?> intento(s). Puedes intentar nuevamente.
            </div>
            <div class="mensaje">
                El puntaje más alto obtenido hasta ahora es <?php echo max($_SESSION['notas_por_curso_me']); ?> puntos.
            </div>
...
<?php elseif ($_SESSION['intentos_por_curso_me'][3] < 3): ?>
    <?php if (!$_SESSION['examen_iniciado']): ?>
        ...
        <input type="button" value="Cerrar y salir" class="modal-button" onclick="window.location.href='me.php'">
    <?php else: ?>
            ...
            <div class="modal-content">
                ...
                <form method="post" id="iniciarExamenForm">
                    <div class="button-container">
                        ...
                        <button type="button" class="modal-button" onclick="window.location.href='me.php '">Volver atrás</button>
                    </div>
                </form>
            </div>
        </div>
