...
function inicializarSesion() {
    if (!isset($_SESSION['intentos_curso_me'])) {
        $_SESSION['intentos_curso_me'] = [];
    }
    
    // ID 2 corresponde al curso de primeros auxilios
    if (!isset($_SESSION['intentos_curso_me'][3])) {
        $_SESSION['intentos_curso_me'][3] = 0;
    }
    
    if (!isset($_SESSION['notas_curso_me'])) {
        $_SESSION['notas_curso_me'] = [];
    }
    if (!isset($_SESSION['notas_curso_me'][3])) {
        $_SESSION['notas_curso_me'][3] = [];
    }
    if (!isset($_SESSION['videotest_completado_curso_me'][3])) {
        $_SESSION['videotest_completado_curso_me'][3] = false;
    }    
    ...
    $_SESSION['videotest_completado_curso_me'][3] = false;
    $_SESSION['nota_final_curso_me'][3] = 0;
    $_SESSION['estado_final_curso_me'][3] = '';
    $_SESSION['nota_mas_alta_curso_me'][3] = 0;
    $_SESSION['id_curso'] = 3; // Curso de Uso y manejo de Extintores
}

// Inicializar sesión si no existe o si se solicita reiniciar
if (!isset($_SESSION['intentos_curso_me']) || isset($_POST['reiniciar'])) {
    inicializarSesion();
}
...
$preguntas = [
    ['¿Qué tipo de extintor se debe utilizar para apagar un fuego de clase A (materiales combustibles sólidos)?', 
     ['Extintor de CO2', 'Extintor de agua', 'Extintor de polvo químico seco'], 
     1, 4],
    ...
    ['¿Qué clase de fuego puede ser apagado con un extintor de CO2?', 
     ['Fuegos eléctricos', 'Fuegos de líquidos inflamables', 'Fuegos de metales'], 
     0, 4],
    ...
    ['¿Qué acción se debe tomar si un extintor no tiene presión suficiente?', 
     ['Utilizarlo con precaución', 'No usarlo y buscar otro extintor', 'Ponerlo a recargar inmediatamente'], 
     1, 4]
];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['respuestas'])) {
    if (tiempoExpirado()) {
        $_SESSION['notas_curso_me'][3][] = 0;
        $_SESSION['estado_final_curso_me'][3] = 'REPROBADO';
        ...
        $_SESSION['notas_curso_me'][3][] = $nota;
        $_SESSION['videotest_iniciado'] = false;

        // Evaluar estado
        if ($nota >= 13) {
            $_SESSION['estado_final_curso_me'][3] = 'APROBADO';
            $_SESSION['nota_final_curso_me'][3] = max($_SESSION['notas_curso_me'][3]);
            $_SESSION['videotest_completado_curso_me'][3] = true;
        } else {
            $_SESSION['estado_final_curso_me'][3] = 'REPROBADO';
        }

        // Actualizar la mejor nota
        if ($nota > $_SESSION['nota_mas_alta_curso_me'][3]) {
            $_SESSION['nota_mas_alta_curso_me'][3] = $nota;
        }

        // Incrementar intento actual en sesión
        $_SESSION['intentos_curso_me'][3]++;
        $intento_actual = $_SESSION['intentos_curso_me'][3];

        // Actualizar base de datos
        if (isset($_SESSION['nota_mas_alta_curso_me'][3]) && isset($_SESSION['DNI'])) {
            ...
            $DNI = $_SESSION['DNI'];
            $nota_mas_alta = $_SESSION['nota_mas_alta_curso_me'][3];

            // Verificar inscripción
            $sqlVerificar = "SELECT id_curso FROM inscripciones WHERE DNI = ? AND id_curso = 3";
        ...
        if ($intento_actual < 3) {
            $mensaje = "Has completado el intento $intento_actual con $nota puntos. Estado: " . $_SESSION['estado_final_curso_me'][3];
        } else {
            $mensaje = "En este último intento tu nota es $nota. La nota final de tus 3 intentos ha sido " . $_SESSION['nota_mas_alta_curso_me'][3];
            $_SESSION['videotest_completado_curso_me'][3] = true;
        }
    }
}
...
if (isset($_POST['reiniciar'])) {
    ...
    $sql = "UPDATE inscripciones SET `intento1` = 0, `intento2` = 0, `intento3` = 0, `nota_videotest` = 0 WHERE DNI = ? AND id_curso = 3";
    $stmt = $conn->prepare($sql);
...
<head>
    ...
    <title>Uso y manejo de extintores</title>
...
<body>
<h1>Videotest de Uso y manejo de extintores</h1>

<div class="modal-background">
<div class="container">

<!-- Verificación si el Videotest ha sido completado -->
<?php if ($_SESSION['videotest_completado_curso_me'][3]): ?>
    <?php if ($_SESSION['estado_final_curso_me'][3] == 'APROBADO'): ?>
        <!-- Mensaje de éxito si se aprueba el examen -->
        <div class="mensaje">
            Felicidades! Ya has aprobado el examen con una nota de <?php echo $_SESSION['nota_final_curso_me'][3]; ?> puntos.
            No necesitas realizar más intentos.
        </div>
        <!-- Botón para volver al curso -->
        <form method="post">
            <button type="button" value="Volver a mi curso" class="volver" onclick="window.location.href='me.php'">Volver a mi curso</button>
        </form>

    <?php else: ?>
        <!-- Si no está aprobado, y se tiene menos de 3 intentos -->
        <?php if ($_SESSION['intentos_curso_me'][3] < 3): ?>
            <div class="mensaje">
                Has reprobado el examen con <?php echo $_SESSION['nota_final']; ?> puntos. Te queda 
                <?php echo 3 - $_SESSION['intentos_curso_me'][3]; ?> intento(s). Puedes intentar nuevamente.
            </div>
            <div class="mensaje">
                El puntaje más alto obtenido hasta ahora es <?php echo max($_SESSION['notas_curso_me']); ?> puntos.
            </div>
        ...
        <?php else: ?>
            ...
            <form method="post">
                <input type="button" value="Volver a mi curso" class="volver" onclick="window.location.href='me.php'">
            </form>
            <div class="mensaje">
...
<?php elseif ($_SESSION['intentos_curso_me'][3] < 3): ?>
