...
function inicializarSesion() {
    if (!isset($_SESSION['intentos_por_curso_Homologacion3G'])) {
        $_SESSION['intentos_por_curso_Homologacion3G'] = [];
    }

    // ID 28 corresponde al curso de Homologación 3G en proceso smaw
    if (!isset($_SESSION['intentos_por_curso_Homologacion3G'][28])) {
        $_SESSION['intentos_por_curso_Homologacion3G'][28] = 0;
    }

    if (!isset($_SESSION['notas_por_curso_Homologacion3G'])) {
        $_SESSION['notas_por_curso_Homologacion3G'] = [];
    }
    if (!isset($_SESSION['notas_por_curso_Homologacion3G'][28])) {
        $_SESSION['notas_por_curso_Homologacion3G'][28] = [];
    }
    if (!isset($_SESSION['examen_completado_por_curso_Homologacion3G'][28])) {
        $_SESSION['examen_completado_por_curso_Homologacion3G'][28] = false;
    }
    ...
    $_SESSION['examen_completado_por_curso_Homologacion3G'][28] = false;
    $_SESSION['nota_final_por_curso_Homologacion3G'][28] = 0;
    $_SESSION['estado_final_por_curso_Homologacion3G'][28] = '';
    $_SESSION['nota_mas_alta_por_curso_Homologacion3G'][28] = 0;
    $_SESSION['id_curso'] = 28; // Curso de Homologación 3G en proceso smaw
}

// Inicializar sesión si no existe o si se solicita reiniciar
if (!isset($_SESSION['intentos_por_curso_Homologacion3G']) || isset($_POST['reiniciar'])) {
    inicializarSesion();
}
...
if (isset($_POST['iniciar_examen'])) {
    if ($_SESSION['intentos_por_curso_Homologacion3G'][28] < 3) {
        $_SESSION['examen_iniciado'] = true;
        $_SESSION['tiempo_inicio'] = time();
        $_SESSION['tiempo_restante'] = $duracion_examen;
        header("Location: examenHomologacion3G.php");
        exit();
    }
}
...
// Obtener las preguntas del examen para el curso de Homologación 3G (id_curso = 28)
$preguntas = obtenerPreguntas(28);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['respuestas'])) {
    if (tiempoExpirado()) {
        $_SESSION['notas_por_curso_Homologacion3G'][28][] = 0;
        ...
        $_SESSION['notas_por_curso_Homologacion3G'][28][] = $nota;
        $_SESSION['examen_iniciado'] = false;
        ...
        if ($nota > $_SESSION['nota_mas_alta_por_curso_Homologacion3G'][28]) {
            $_SESSION['nota_mas_alta_por_curso_Homologacion3G'][28] = $nota;
        }

        // Incrementar intento actual en sesión
        $_SESSION['intentos_por_curso_Homologacion3G'][28]++;
        $intento_actual = $_SESSION['intentos_por_curso_Homologacion3G'][28];

        // Actualizar base de datos
        if (isset($_SESSION['nota_mas_alta_por_curso_Homologacion3G'][28]) && isset($_SESSION['DNI'])) {
            $conn = new mysqli('localhost', 'root', '', 'usuario');
            if ($conn->connect_error) {
                die("Error de conexión: " . $conn->connect_error);
            }
            ...
            $DNI = $_SESSION['DNI'];
            $nota_mas_alta = $_SESSION['nota_mas_alta_por_curso_Homologacion3G'][28];
        ...
        if ($intento_actual < 3) {
            $mensaje = "Has completado el intento $intento_actual con $nota puntos. Estado: " . $_SESSION['estado_final_por_curso_Homologacion3G'][28];
        } else {
            $mensaje = "En este último intento tu nota es $nota. La nota final de tus 3 intentos ha sido " . $_SESSION['nota_mas_alta_por_curso_Homologacion3G'][28];
            $_SESSION['examen_completado_por_curso_Homologacion3G'][28] = true;
        }
    }
}
...
if (isset($_POST['reiniciar'])) {
    ...
    $sql = "UPDATE inscripciones SET `intentose1` = 0, `intentose2` = 0, `intentose3` = 0, `examen_final` = 0 WHERE DNI = ? AND id_curso = 28";
    $stmt = $conn->prepare($sql);
...
<head>
    ...
    <title>Examen de Homologación 3G en proceso smaw</title>
...
<body>
    ...
    <div class="container">
        <h1>Examen de Homologación 3G en proceso smaw</h1>
    <?php if ($_SESSION['examen_completado_por_curso_Homologacion3G'][28]): ?>
    <?php if ($_SESSION['estado_final_por_curso_Homologacion3G'][28] == 'APROBADO'): ?>
            <div class="mensaje">
    ...
    <?php else: ?>
            <?php if ($_SESSION['intentos_por_curso_Homologacion3G'][28] < 3): ?>
                <div class="mensaje">
                    Has reprobado el examen con <?php echo $_SESSION['nota_final_por_curso_Homologacion3G'][28]; ?> puntos. Te queda
                    <?php echo 3 - $_SESSION['intentos_por_curso_Homologacion3G'][28]; ?> intento(s). Puedes intentar nuevamente.
                </div>
                <div class="mensaje">
                    El puntaje más alto obtenido hasta ahora es <?php echo max($_SESSION['notas_por_curso_Homologacion3G'][28] ?? [0]); ?> puntos.
                </div>
...
<?php elseif ($_SESSION['intentos_por_curso_Homologacion3G'][28] < 3): ?>
...
<div id="inicioModal" class="modal">
    <div class="modal-content">
        ...
        <div class="button-container">
            ...
            <button type="button" class="modal-button" onclick="window.location.href='Homologacion3G.php'">Volver atrás</button>
        </div>
        </form>
    </div>
</div>
