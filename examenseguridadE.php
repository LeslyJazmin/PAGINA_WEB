...
function inicializarSesion() {
    if (!isset($_SESSION['intentos_por_curso_seguridadE'])) {
        $_SESSION['intentos_por_curso_seguridadE'] = [];
    }
    
    // ID 2 corresponde al curso de primeros auxilios
    if (!isset($_SESSION['intentos_por_curso_seguridadE'][20])) {
        $_SESSION['intentos_por_curso_seguridadE'][20] = 0;
    }
    
    if (!isset($_SESSION['notas_por_curso_seguridadE'])) {
        $_SESSION['notas_por_curso_seguridadE'] = [];
    }
    if (!isset($_SESSION['notas_por_curso_seguridadE'][20])) {
        $_SESSION['notas_por_curso_seguridadE'][20] = [];
    }
    if (!isset($_SESSION['examen_completado_por_curso_seguridadE'][20])) {
        $_SESSION['examen_completado_por_curso_seguridadE'][20] = false;
    }    
    ...
    $_SESSION['examen_completado_por_curso_seguridadE'][20] = false;
    $_SESSION['nota_final_por_curso_seguridadE'][20] = 0;
    $_SESSION['estado_final_por_curso_seguridadE'][20] = '';
    $_SESSION['nota_mas_alta_por_curso_seguridadE'][20] = 0;
    $_SESSION['id_curso'] = 20; // Curso de primeros auxilios
}

// Inicializar sesión si no existe o si se solicita reiniciar
if (!isset($_SESSION['intentos_por_curso_seguridadE']) || isset($_POST['reiniciar'])) {
    inicializarSesion();
}
...
if (isset($_POST['iniciar_examen'])) {
    if ($_SESSION['intentos_por_curso_seguridadE'][20] < 3) {
        $_SESSION['examen_iniciado'] = true;
        $_SESSION['tiempo_inicio'] = time();
        $_SESSION['tiempo_restante'] = $duracion_examen;
        header("Location: examenseguridadE.php");
        exit();
    }
}
...
$preguntas = [
    ['¿Cuál es la principal medida de seguridad antes de trabajar con electricidad?', 
     ['Desenergizar y bloquear la fuente de energía', 'Usar guantes de algodón', 'Trabajar rápido para minimizar el riesgo'], 
     0], // Respuesta correcta: "Desenergizar y bloquear la fuente de energía" (índice 0)
    
    ['¿Qué equipo de protección personal es obligatorio para trabajos eléctricos?', 
     ['Guantes dieléctricos, casco y botas aislantes', 'Guantes de tela y lentes de sol', 'Chaleco reflectante'], 
     0], // Respuesta correcta: "Guantes dieléctricos, casco y botas aislantes" (índice 0)
    
    ['¿Cuál es el procedimiento correcto antes de reparar un equipo eléctrico?', 
     ['Etiquetar y bloquear la fuente de energía', 'Apagar el interruptor sin necesidad de señalización', 'Revisar el equipo mientras está energizado'], 
     0], // Respuesta correcta: "Etiquetar y bloquear la fuente de energía" (índice 0)
    
    ['¿Qué hacer si alguien sufre una descarga eléctrica?', 
     ['Tocar a la persona para apartarla', 'Cortar la corriente y llamar a emergencias', 'Rociar agua para enfriar la zona afectada'], 
     1], // Respuesta correcta: "Cortar la corriente y llamar a emergencias" (índice 1)
    
    ['¿Cuál es la distancia mínima de seguridad al trabajar cerca de líneas eléctricas de alta tensión?', 
     ['1 metro', '3 metros', '10 metros'], 
     1] // Respuesta correcta: "3 metros" (índice 1)
];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['respuestas'])) {
    if (tiempoExpirado()) {
        $_SESSION['notas_por_curso_seguridadE'][20][] = 0;
        $_SESSION['estado_final_por_curso_seguridadE'][20] = 'REPROBADO';
        ...
        $_SESSION['notas_por_curso_seguridadE'][20][] = $nota;
        $_SESSION['examen_iniciado'] = false;

        // Evaluar estado
        if ($nota >= 13) {
            $_SESSION['estado_final_por_curso_seguridadE'][20] = 'APROBADO';
            $_SESSION['nota_final_por_curso_seguridadE'][20] = $_SESSION['nota_final_por_curso_seguridadE'][20];
            $_SESSION['examen_completado_por_curso_seguridadE'][20] = true;
        } else {
            $_SESSION['estado_final_por_curso_seguridadE'][20] = 'REPROBADO';
        }

        // Actualizar la mejor nota
        if ($nota > $_SESSION['nota_mas_alta_por_curso_seguridadE'][20]) {
            $_SESSION['nota_mas_alta_por_curso_seguridadE'][20] = $nota;
        }

        // Incrementar intento actual en sesión
        $_SESSION['intentos_por_curso_seguridadE'][20]++;
        $intento_actual = $_SESSION['intentos_por_curso_seguridadE'][20];

        // Actualizar base de datos
        if (isset($_SESSION['nota_mas_alta_por_curso_seguridadE'][20]) && isset($_SESSION['DNI'])) {
            ...
            $DNI = $_SESSION['DNI'];
            $nota_mas_alta = $_SESSION['nota_mas_alta_por_curso_seguridadE'][20];
        ...
        if ($intento_actual < 3) {
            $mensaje = "Has completado el intento $intento_actual con $nota puntos. Estado: " . $_SESSION['estado_final_por_curso_seguridadE'][20];
        } else {
            $mensaje = "En este último intento tu nota es $nota. La nota final de tus 3 intentos ha sido " . $_SESSION['nota_mas_alta_por_curso_seguridadE'][20];
            $_SESSION['examen_completado_por_curso_seguridadE'][20] = true;
        }
    }
}
...
if (isset($_POST['reiniciar'])) {
    ...
    $sql = "UPDATE inscripciones SET `intentose1` = 0, `intentose2` = 0, `intentose3` = 0, `examen_final` = 0 WHERE DNI = ? AND id_curso = 20";
    $stmt = $conn->prepare($sql);
...
<head>
    ...
    <title>Examen de Seguridad para Trabajos Eléctricos</title>
...
<body>
    ...
    <div class="container">
        <h1>Examen de Seguridad para Trabajos Eléctricos</h1>
    <?php if ($_SESSION['examen_completado_por_curso_seguridadE'][20]): ?>
    <?php if ($_SESSION['estado_final_por_curso_seguridadE'][20] == 'APROBADO'): ?>
    ...
    <?php else: ?>
        ...
        <?php if ($_SESSION['intentos_por_curso_seguridadE'][20] < 3): ?>
            <div class="mensaje">
                Has reprobado el examen con <?php echo $_SESSION['nota_final']; ?> puntos. Te queda 
                <?php echo 3 - $_SESSION['intentos_por_curso_seguridadE'][20]; ?> intento(s). Puedes intentar nuevamente.
            </div>
            <div class="mensaje">
                El puntaje más alto obtenido hasta ahora es <?php echo max($_SESSION['notas_por_curso_seguridadE']); ?> puntos.
            </div>
...
<?php elseif ($_SESSION['intentos_por_curso_seguridadE'][20] < 3): ?>
...
<div id="inicioModal" class="modal">
    <div class="modal-content">
                ...
                <div class="button-container">
                        ...
                        <button type="button" class="modal-button" onclick="window.location.href='seguridadE.php'">Volver atrás</button>
            </div>
        </form>
    </div>
</div>
