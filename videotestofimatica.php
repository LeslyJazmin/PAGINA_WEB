...
function inicializarSesion() {
    if (!isset($_SESSION['intentos_curso_ofimatica'])) {
        $_SESSION['intentos_curso_ofimatica'] = [];
    }
    
    // ID 2 corresponde al curso de primeros auxilios
    if (!isset($_SESSION['intentos_curso_ofimatica'][27])) {
        $_SESSION['intentos_curso_ofimatica'][27] = 0;
    }
    
    if (!isset($_SESSION['notas_curso_ofimatica'])) {
        $_SESSION['notas_curso_ofimatica'] = [];
    }
    if (!isset($_SESSION['notas_curso_ofimatica'][27])) {
        $_SESSION['notas_curso_ofimatica'][27] = [];
    }
    if (!isset($_SESSION['videotest_completado_curso_ofimatica'][27])) {
        $_SESSION['videotest_completado_curso_ofimatica'][27] = false;
    }    
    ...
    $_SESSION['videotest_completado_curso_ofimatica'][27] = false;
    $_SESSION['nota_final_curso_ofimatica'][27] = 0;
    $_SESSION['estado_final_curso_ofimatica'][27] = '';
    $_SESSION['nota_mas_alta_curso_ofimatica'][27] = 0;
    $_SESSION['id_curso'] = 27; // Curso de Uso y manejo de Extintores
}

// Inicializar sesión si no existe o si se solicita reiniciar
if (!isset($_SESSION['intentos_curso_ofimatica']) || isset($_POST['reiniciar'])) {
    inicializarSesion();
}
...
if (isset($_POST['iniciar_videotest'])) {
    if ($_SESSION['intentos_curso_ofimatica'][27] < 3) {
        $_SESSION['videotest_iniciado'] = true;
        $_SESSION['tiempo_inicio'] = time();
        $_SESSION['tiempo_restante'] = $duracion_examen;
        header("Location: videotestofimatica.php");
        exit();
    }
}
...
$preguntas = [
    ['¿Cuál es el atajo de teclado para copiar en Microsoft Word?', 
     ['Ctrl + X', 'Ctrl + C', 'Ctrl + V'], 
     1, 4], // Respuesta correcta: "Ctrl + C"

    ['¿Qué software de ofimática se usa principalmente para crear hojas de cálculo?', 
     ['Microsoft Word', 'Microsoft Excel', 'Microsoft PowerPoint'], 
     1, 4], // Respuesta correcta: "Microsoft Excel"

    ['¿Cuál es la función de la combinación Ctrl + Z en programas de ofimática?', 
     ['Rehacer una acción', 'Deshacer una acción', 'Guardar el documento'], 
     1, 4], // Respuesta correcta: "Deshacer una acción"

    ['¿Qué tipo de archivo genera Microsoft Excel por defecto?', 
     ['.docx', '.xlsx', '.pptx'], 
     1, 4], // Respuesta correcta: ".xlsx"

    ['¿Cuál de las siguientes opciones permite insertar una diapositiva en Microsoft PowerPoint?', 
     ['Ctrl + M', 'Ctrl + N', 'Ctrl + P'], 
     0, 4] // Respuesta correcta: "Ctrl + M"
];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['respuestas'])) {
    if (tiempoExpirado()) {
        $_SESSION['notas_curso_ofimatica'][27][] = 0;
        $_SESSION['estado_final_curso_ofimatica'][27] = 'REPROBADO';
        ...
        $_SESSION['notas_curso_ofimatica'][27][] = $nota;
        $_SESSION['videotest_iniciado'] = false;

        // Evaluar estado
        if ($nota >= 13) {
            $_SESSION['estado_final_curso_ofimatica'][27] = 'APROBADO';
            $_SESSION['nota_final_curso_ofimatica'][27] = max($_SESSION['notas_curso_ofimatica'][27]);
            $_SESSION['videotest_completado_curso_ofimatica'][27] = true;
        } else {
            $_SESSION['estado_final_curso_ofimatica'][27] = 'REPROBADO';
        }

        // Actualizar la mejor nota
        if ($nota > $_SESSION['nota_mas_alta_curso_ofimatica'][27]) {
            $_SESSION['nota_mas_alta_curso_ofimatica'][27] = $nota;
        }

        // Incrementar intento actual en sesión
        $_SESSION['intentos_curso_ofimatica'][27]++;
        $intento_actual = $_SESSION['intentos_curso_ofimatica'][27];

        // Actualizar base de datos
        if (isset($_SESSION['nota_mas_alta_curso_ofimatica'][27]) && isset($_SESSION['DNI'])) {
            ...
            $DNI = $_SESSION['DNI'];
            $nota_mas_alta = $_SESSION['nota_mas_alta_curso_ofimatica'][27];

            // Verificar inscripción
            $sqlVerificar = "SELECT id_curso FROM inscripciones WHERE DNI = ? AND id_curso = 27";
        ...
        if ($intento_actual < 3) {
            $mensaje = "Has completado el intento $intento_actual con $nota puntos. Estado: " . $_SESSION['estado_final_curso_ofimatica'][27];
        } else {
            $mensaje = "En este último intento tu nota es $nota. La nota final de tus 3 intentos ha sido " . $_SESSION['nota_mas_alta_curso_ofimatica'][27];
            $_SESSION['videotest_completado_curso_ofimatica'][27] = true;
        }
    }
}
...
if (isset($_POST['reiniciar'])) {
    ...
    $sql = "UPDATE inscripciones SET `intento1` = 0, `intento2` = 0, `intento3` = 0, `nota_videotest` = 0 WHERE DNI = ? AND id_curso = 27";
    $stmt = $conn->prepare($sql);
...
<body>
<h1>Videotest de Ofimática</h1>

<div class="modal-background">
<div class="container">

<!-- Verificación si el Videotest ha sido completado -->
<?php if ($_SESSION['videotest_completado_curso_ofimatica'][27]): ?>
    <?php if ($_SESSION['estado_final_curso_ofimatica'][27] == 'APROBADO'): ?>
        <!-- Mensaje de éxito si se aprueba el examen -->
        <div class="mensaje">
            Felicidades! Ya has aprobado el examen con una nota de <?php echo $_SESSION['nota_final_curso_ofimatica'][27]; ?> puntos.
            No necesitas realizar más intentos.
        </div>
        <!-- Botón para volver al curso -->
        <form method="post">
            <button type="button" value="Volver a mi curso" class="volver" onclick="window.location.href='ofimatica.php'">Volver a mi curso</button>
        </form>

    <?php else: ?>
        <!-- Si no está aprobado, y se tiene menos de 3 intentos -->
        <?php if ($_SESSION['intentos_curso_ofimatica'][27] < 3): ?>
            <div class="mensaje">
                Has reprobado el examen con <?php echo $_SESSION['nota_final']; ?> puntos. Te queda 
                <?php echo 3 - $_SESSION['intentos_curso_ofimatica'][27]; ?> intento(s). Puedes intentar nuevamente.
            </div>
            <div class="mensaje">
                El puntaje más alto obtenido hasta ahora es <?php echo max($_SESSION['notas_curso_ofimatica']); ?> puntos.
            </div>
        ...
        <?php else: ?>
            ...
            <form method="post">
                <input type="button" value="Volver a mi curso" class="volver" onclick="window.location.href='ofimatica.php'">
            </form>
            <div class="mensaje">
...
<?php elseif ($_SESSION['intentos_curso_ofimatica'][27] < 3): ?>
    <?php if (!$_SESSION['videotest_iniciado']): ?>
        ...
        <input type="button" value="Cerrar y salir" class="modal-button" onclick="window.location.href='ofimatica.php'">
    <?php else: ?>
    ...
    <div class="modal-content">
        ...
        <form method="post" id="iniciarExamenForm">
            <div class="button-container">
                ...
                <button type="button" class="modal-button" onclick="window.location.href='ofimatica.php '">Cancelar</button>
            </div>
        </form>
    </div>
</div>
