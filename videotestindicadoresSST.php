...
function inicializarSesion() {
    ...
    $_SESSION['id_curso'] = 23; // Curso de indicadoresSST
}
...
if (isset($_POST['iniciar_videotest'])) {
    if ($_SESSION['intentos_curso_indicadoresSST'][23] < 3) {
        $_SESSION['videotest_iniciado'] = true;
        $_SESSION['tiempo_inicio'] = time();
        $_SESSION['tiempo_restante'] = $duracion_examen;
        header("Location: videotestindicadoresSST.php");
        exit();
    }
}
...
$sql_preguntas = "SELECT id, pregunta FROM preguntas WHERE id_curso = 23";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['respuestas'])) {
    if (tiempoExpirado()) {
        ...
        if ($nota >= 13) {
            ...
            $_SESSION['nota_final_curso_indicadoresSST'][23] = max($_SESSION['notas_curso_indicadoresSST'][23]);
        ...
        if (isset($_SESSION['nota_mas_alta_curso_indicadoresSST'][23]) && isset($_SESSION['DNI'])) {
            $conn = new mysqli('localhost', 'root', '', 'usuario');
            if ($conn->connect_error) {
                die("Error de conexión: " . $conn->connect_error);
            }
            ...
            $DNI = $_SESSION['DNI'];
            $nota_mas_alta = $_SESSION['nota_mas_alta_curso_indicadoresSST'][23];
        ...
        if ($intento_actual < 3) {
            ...
            $mensaje = "En este último intento tu nota es $nota. La nota final de tus 3 intentos ha sido " . $_SESSION['nota_mas_alta_curso_indicadoresSST'][23];
            $_SESSION['videotest_completado_curso_indicadoresSST'][23] = true;
        }
    }
}
...
if (isset($_POST['reiniciar'])) {
    ...
    $sql = "UPDATE inscripciones SET `intento1` = 0, `intento2` = 0, `intento3` = 0, `nota_videotest` = 0 WHERE DNI = ? AND id_curso = 23";
    $stmt = $conn->prepare($sql);
...
<?php if ($_SESSION['videotest_completado_curso_indicadoresSST'][23]): ?>
    <?php if ($_SESSION['estado_final_curso_indicadoresSST'][23] == 'APROBADO'): ?>
        ...
        <div class="mensaje">
            Felicidades! Ya has aprobado el examen con una nota de <?php 
            echo $_SESSION['nota_final_curso_indicadoresSST'][23]; ?> puntos.
    ...
    <div class="modal-content">
        ...
        <form method="post" id="iniciarExamenForm">
            <div class="button-container">
                ...
                <button type="button" class="modal-button" onclick="window.location.href='indicadoresSST.php '">Cancelar</button>
            </div>
        </form>
    </div>
</div>
...
window.addEventListener('focus', function() {
    ...
    <div class="modal-content">
        ...
        <button onclick="window.location.href='indicadoresSST.php'">Cerrar</button>
    </div>
</div>
</body>
</html>    
