...
function inicializarSesion() {
    if (!isset($_SESSION['intentos_por_curso_trabajosC'])) {
        $_SESSION['intentos_por_curso_trabajosC'] = [];
    }
    
    // ID 2 corresponde al curso de primeros auxilios
    if (!isset($_SESSION['intentos_por_curso_trabajosC'][25])) {
        $_SESSION['intentos_por_curso_trabajosC'][25] = 0;
    }
    
    if (!isset($_SESSION['notas_por_curso_trabajosC'])) {
        $_SESSION['notas_por_curso_trabajosC'] = [];
    }
    if (!isset($_SESSION['notas_por_curso_trabajosC'][25])) {
        $_SESSION['notas_por_curso_trabajosC'][25] = [];
    }
    if (!isset($_SESSION['examen_completado_por_curso_trabajosC'][25])) {
        $_SESSION['examen_completado_por_curso_trabajosC'][25] = false;
    }    
    ...
    $_SESSION['examen_completado_por_curso_trabajosC'][25] = false;
    $_SESSION['nota_final_por_curso_trabajosC'][25] = 0;
    $_SESSION['estado_final_por_curso_trabajosC'][25] = '';
    $_SESSION['nota_mas_alta_por_curso_trabajosC'][25] = 0;
    $_SESSION['id_curso'] = 25; // Curso de primeros auxilios
}

// Inicializar sesión si no existe o si se solicita reiniciar
if (!isset($_SESSION['intentos_por_curso_trabajosC']) || isset($_POST['reiniciar'])) {
    inicializarSesion();
}
...
if (isset($_POST['iniciar_examen'])) {
    if ($_SESSION['intentos_por_curso_trabajosC'][25] < 3) {
        $_SESSION['examen_iniciado'] = true;
        $_SESSION['tiempo_inicio'] = time();
        $_SESSION['tiempo_restante'] = $duracion_examen;
        header("Location: examentrabajosC.php");
        exit();
    }
}
...
$preguntas = [
    ['¿Cuál es el principal equipo de protección personal para trabajos en caliente?', 
     ['Casco con pantalla facial y guantes resistentes al calor', 'Chaleco reflectante', 'Gafas de sol'], 
     0], // Respuesta correcta: "Casco con pantalla facial y guantes resistentes al calor" (índice 0)
    
    ['¿Qué se debe hacer antes de iniciar un trabajo en caliente?', 
     ['Asegurar que haya un extintor cerca y retirar materiales inflamables', 'Encender la máquina y empezar a trabajar', 'No se requiere ninguna preparación especial'], 
     0], // Respuesta correcta: "Asegurar que haya un extintor cerca y retirar materiales inflamables" (índice 0)
    
    ['¿Cuál es el tiempo mínimo de vigilancia después de terminar un trabajo en caliente?', 
     ['5 minutos', '30 minutos', 'No es necesario vigilar'], 
     1], // Respuesta correcta: "30 minutos" (índice 1)
    
    ['¿Qué tipo de permiso se necesita antes de realizar trabajos en caliente?', 
     ['Permiso de trabajo en caliente', 'Permiso de acceso al área', 'No se necesita ningún permiso'], 
     0], // Respuesta correcta: "Permiso de trabajo en caliente" (índice 0)
    
    ['¿Cuál es la función del vigilante de fuego en un trabajo en caliente?', 
     ['Realizar la soldadura', 'Supervisar el área y actuar en caso de incendio', 'Controlar el acceso al área'], 
     1] // Respuesta correcta: "Supervisar el área y actuar en caso de incendio" (índice 1)
];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['respuestas'])) {
    if (tiempoExpirado()) {
        $_SESSION['notas_por_curso_trabajosC'][25][] = 0;
        $_SESSION['estado_final_por_curso_trabajosC'][25] = 'REPROBADO';
        ...
        $_SESSION['notas_por_curso_trabajosC'][25][] = $nota;
        $_SESSION['examen_iniciado'] = false;

        // Evaluar estado
        if ($nota >= 13) {
            $_SESSION['estado_final_por_curso_trabajosC'][25] = 'APROBADO';
            $_SESSION['nota_final_por_curso_trabajosC'][25] = $_SESSION['nota_final_por_curso_trabajosC'][25];
            $_SESSION['examen_completado_por_curso_trabajosC'][25] = true;
        } else {
            $_SESSION['estado_final_por_curso_trabajosC'][25] = 'REPROBADO';
        }

        // Actualizar la mejor nota
        if ($nota > $_SESSION['nota_mas_alta_por_curso_trabajosC'][25]) {
            $_SESSION['nota_mas_alta_por_curso_trabajosC'][25] = $nota;
        }

        // Incrementar intento actual en sesión
        $_SESSION['intentos_por_curso_trabajosC'][25]++;
        $intento_actual = $_SESSION['intentos_por_curso_trabajosC'][25];

        // Actualizar base de datos
        if (isset($_SESSION['nota_mas_alta_por_curso_trabajosC'][25]) && isset($_SESSION['DNI'])) {
            ...
            $DNI = $_SESSION['DNI'];
            $nota_mas_alta = $_SESSION['nota_mas_alta_por_curso_trabajosC'][25];

            // Verificar inscripción
            $sqlVerificar = "SELECT id_curso FROM inscripciones WHERE DNI = ? AND id_curso = 25";
        ...
        if ($intento_actual < 3) {
            $mensaje = "Has completado el intento $intento_actual con $nota puntos. Estado: " . $_SESSION['estado_final_por_curso_trabajosC'][25];
        } else {
            $mensaje = "En este último intento tu nota es $nota. La nota final de tus 3 intentos ha sido " . $_SESSION['nota_mas_alta_por_curso_trabajosC'][25];
            $_SESSION['examen_completado_por_curso_trabajosC'][25] = true;
        }
    }
}
...
if (isset($_POST['reiniciar'])) {
    ...
    $sql = "UPDATE inscripciones SET `intentose1` = 0, `intentose2` = 0, `intentose3` = 0, `examen_final` = 0 WHERE DNI = ? AND id_curso = 25";
    $stmt = $conn->prepare($sql);
...
<head>
    ...
    <title>Examen de Seguridad para Trabajos en Caliente</title>
...
<body>
    ...
    <div class="container">
        <h1>Examen de Seguridad para Trabajos en Caliente</h1>
    <?php if ($_SESSION['examen_completado_por_curso_trabajosC'][25]): ?>
    <?php if ($_SESSION['estado_final_por_curso_trabajosC'][25] == 'APROBADO'): ?>
        ...
        <form method="post">
            <button type="button" value="Volver a mi curso" class="volver" onclick="window.location.href='trabajosC.php'">Volver a mi curso</button>
        </form>

    <?php else: ?>
        <!-- Si no está aprobado, y se tiene menos de 3 intentos -->
        <?php if ($_SESSION['intentos_por_curso_trabajosC'][25] < 3): ?>
            <div class="mensaje">
                Has reprobado el examen con <?php echo $_SESSION['nota_final']; ?> puntos. Te queda 
                <?php echo 3 - $_SESSION['intentos_por_curso_trabajosC'][25]; ?> intento(s). Puedes intentar nuevamente.
            </div>
            <div class="mensaje">
                El puntaje más alto obtenido hasta ahora es <?php echo max($_SESSION['notas_por_curso_trabajosC']); ?> puntos.
            </div>
        ...
        <?php else: ?>
            ...
            <form method="post">
                <input type="button" value="Volver a mi curso" class="volver" onclick="window.location.href='trabajosC.php'">
            </form>
            <div class="mensaje">
...
<?php elseif ($_SESSION['intentos_por_curso_trabajosC'][25] < 3): ?>
...
<div id="inicioModal" class="modal">
    <div class="modal-content">
                ...
                <div class="button-container">
                        ...
                        <button type="button" class="modal-button" onclick="window.location.href='trabajosC.php'">Volver atrás</button>
            </div>
        </form>
    </div>
</div>
